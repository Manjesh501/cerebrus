...
class BatchProcessor:
    ...
    def _process_single_file(self, file_path, batch_dir):
        ...
        if 'ml_model' in self.analyzers and file_extension in ['.exe', '.dll']:
            try:
                ...
                analysis_results['ml_model'] = {
                    'is_malware': bool(prediction[0] == 1),  # Ensure this is a proper boolean
                    'prediction': int(prediction[0])  # Store as int for JSON compatibility
                }
        ...
        with open(result_file, 'w') as f:
            json.dump(serializable_result, f, indent=2, cls=NumpyJSONEncoder)
