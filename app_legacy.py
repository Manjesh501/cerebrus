...
try:
    import magic
    import mimetypes  # keep as fallback
except ImportError:
    magic = None
    import mimetypes

# Load secrets from .env (if present — never commit the real .env)
load_dotenv()
...
# Setup logging — write to file AND console so issues are visible during development
logging.basicConfig(
    level=logging.INFO,
...
# Secret key required for sessions and flash messages — read from environment
...
def init_components():
    ...
    try:
        ...
        if not vt_api_key:
            logger.warning("VT_API_KEY not set — VirusTotal scanning disabled. Set it in .env")
...
def allowed_file(filename, header_bytes=None):
    """Check if a file is allowed by extension, with optional MIME magic validation.

    Pass header_bytes (first 2048 bytes of the file) to also validate that the
    actual file content matches the declared extension category.
    """
    if '.' not in filename:
        return False
    ...
    if magic and header_bytes is not None:
        ...
        exec_exts = ALLOWED_EXTENSIONS.get('executable', set()) | ALLOWED_EXTENSIONS.get('malware', set())
        if mime in executable_mimes and ext not in exec_exts:
            logger.warning(f"MIME mismatch for '{filename}': ext=.{ext} but magic={mime}")
            return False
    return True
...
def analyze():
    ...
    if file and allowed_file(file.filename, file.read(2048) or None):
        file.seek(0)  # Reset stream after reading header for MIME check
        try:
            ...
            if not os.path.abspath(file_path).startswith(abs_upload + os.sep):
...
def export_batch_results(timestamp, export_format):
    """Export batch analysis results in various formats"""
    # Validate timestamp to prevent path traversal via URL parameter
...
def fallback_extract_features(file_path):
    """Simple fallback feature extractor used when the primary extractor is unavailable."""
    ...
    # File type via magic or extension heuristic
    try:
        if magic:
            file_type = magic.from_file(file_path)
            features['is_executable'] = 1 if 'executable' in file_type.lower() else 0
            features['is_dll'] = 1 if 'dll' in file_type.lower() else 0
        else:
            raise RuntimeError("magic not available")
    except Exception as e:
        logger.debug(f"Magic detection failed for {file_path}: {e}")
        features['is_executable'] = 1 if file_path.lower().endswith('.exe') else 0
        features['is_dll'] = 1 if file_path.lower().endswith('.dll') else 0
    ...
    try:
        ...
        logger.debug(f"Entropy calculation failed for {file_path}: {e}")
        features['entropy'] = 0.0
    ...
    try:
        ...
        features['md5_hash'] = int(md5[:8], 16) / (1 << 32)
    except Exception as e:
        logger.debug(f"Hash feature failed for {file_path}: {e}")
...
def fallback_detect_malware(file_path):
    ...
    # Decision factors — any single strong signal is enough
    high_entropy = entropy > 7.0       # Likely encrypted or packed
    strange_size = file_size < 1000 or file_size > 20_000_000
    many_patterns = suspicious_matches > 5  # Multiple suspicious API calls

    # Flag if: many API patterns alone, OR high entropy combined with size anomaly
    is_suspicious = many_patterns or (high_entropy and strange_size)

    logger.debug(
        f"Fallback detection: entropy={entropy:.2f}, patterns={suspicious_matches}, "
...
if __name__ == '__main__':
    ...
    if args.debug:
        logger.warning("Starting in DEBUG mode — do not use in production")
